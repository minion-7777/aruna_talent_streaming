...
async function refreshStreamCount(streamId: string): Promise<number> {
  const count = await redis.get(`viewers:${streamId}`);
  return count ? parseInt(count, 10) : 0;
}

/** Count a viewer once per IP per stream within SESSION_TTL_SEC (WS + HLS share this). */
export async function recordViewer(
  ...
  if (!isNew) return refreshStreamCount(streamId);
...
export async function removeViewer(
  ...
  if (!hadSession) return refreshStreamCount(streamId);
